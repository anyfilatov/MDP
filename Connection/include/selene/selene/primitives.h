...
{
    return _get(_id<T>(), L, N);
}

template< typename... Args, std::size_t... Ns >
std::tuple< Args... > member_stub_impl( lua_State* L, const indices< Ns... >& )
{
  std::tuple< Args... > args( get_arg< Args >( L, Ns + 1 )... );
  return args;
...
std::tuple< Args... >  member_stub( lua_State* L )
{
...
inline T* _get(_id<T*>, lua_State *l, const int index) {
    std::cout << "From topointer" << std::endl;
    auto t = lua_topointer(l, index);
    return (T*)(t);
}

template <typename... T>
inline std::tuple<T...> _get(_id<std::tuple<T...>>, lua_State *l, const int index) {
    std::cout << "From tuple" << std::endl;
    return member_stub<T...>(l);
}
