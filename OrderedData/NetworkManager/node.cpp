...
Node::Node(QString host, qint16 port, qint32 heartbeat, qint32 cleanupInterval, NetworkManager& manager) {
    _host = host;
    _port = port;
    ...
    connect(this, SIGNAL(heartbeatChanged()), this, SLOT(handleHeartbeatChanged()));
    connect(&_timer, SIGNAL(timeout()), this, SLOT(handleTimeout()));
    connect(this, SIGNAL(notifyManager(QString)), &manager, SLOT(handleDeadNode(QString)));
}
...
//    connect(this, SIGNAL(heartbeatChanged()), this, SLOT(handleHeartbeatChanged()));
//    connect(_timer, SIGNAL(timeout()), this, SLOT(handleTimeout()));
//    connect(this, SIGNAL(notifyManager(QString)), &manager, SLOT(handleDeadNode()));
}
...
qint16 Node::getPort() const {
    return _port;
}

qint32 Node::getHeartbeat() const {
    return _heartbeat;
}
...
const QTimer* Node::getTimer() const {
    return &_timer;
}
...
void Node::handleHeartbeatChanged() {
    _timer.start();
}

void Node::handleTimeout() {
    emit notifyManager(getAddress());
}
