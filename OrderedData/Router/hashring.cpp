...
HashRing::HashRing(NetworkManager* manager, iCache<QString, QString> *cache, QObject *parent) : QObject(parent)
{
    _manager = manager;
    _cache = cache;
    ...
    _hasMyReplica = _ring[(_ring.indexOf(_manager->getMyself()) + 1) % _ring.size()];
    // =============================================
...
void HashRing::stabilize()
{
    ...
    to_be_successor = _ring[(_ring.indexOf(_manager->getMyself()) + 1) % _ring.size()];
    ...
    // get all primary keys and send them (i.e. update)  to to_be_successor[i]
    qDebug() << "UPDATE REPLICA KEYS. From " << _manager->getMyself()->getAddress() << " to " << to_be_successor->getAddress();
